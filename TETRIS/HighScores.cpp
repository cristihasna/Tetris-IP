...
void HighScores::processScore(int score, float minutesElapsed, float secondsElapsed, char timestamp[100], Score highScores[MAX_H_SCORES]) {
...
void HighScores::transformHighScores(Score highScores[MAX_H_SCORES]) {
	...
	for (int i = 0; i < 10; i++) {
		...
		if (i % 2 == 0) {
			HighScores::scores[i].setString(std::to_string(HighScores::highScores[i / 2].score));
